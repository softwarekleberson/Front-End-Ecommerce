...
async function loadSeparationOrders() {
	...
	try {
		...
		if (!response.ok) {
			const errorBody = await response.text();
			throw new Error(`Error ${response.status}: ${errorBody || "Unable to fetch separation orders"}`);
		}
		...
		console.error("Error loading separation orders:", error);
		if (ordersSections) {
			ordersSections.innerHTML = "<p>Error loading orders</p>";
		}
		if (totalOrdersElement) {
			totalOrdersElement.innerHTML = "<p>Total Separation: 0</p>";
		}
	}
}
